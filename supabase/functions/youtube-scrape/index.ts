...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (accountId) {
      ...
      const engagementRate = data.subscribersCount && data.subscribersCount > 0
        ? ((totalLikes + totalComments) / data.subscribersCount / Math.max(totalPosts, 1)) * 100
        : 0;
      ...
      const { error: creatorMetricsError } = await supabase
        ...
        .upsert({
          ...
          display_name: data.displayName,
          profile_image_url: data.profileImageUrl,
          followers_count: data.subscribersCount,
